#Unit 13 Using Web Services Exercises 
...
#Accesses the formatted address value. 
    ...
    #Goes through all the address components that exist for the location. 
    #Each address_component index is a dictionary. 
    #Thus item is representing a dictonary, and the for loop is going through each dictionary in js['results'][0]['address_components']
    #'results' and 'address_components' are also dictionaries. 
    #    print (item)
    #print ('')
        if item['types'] == ['country', 'political']: 
    #When going through a specific address component dictionary, it checks to see if the key 'types' is associated with the list value of ['country', 'political'].
    #Code below only goes through when the key and value is matched. 
    #    print ('found!')
            x = item['short_name']
        #Stores country code as variable called x. 
        
    #print (x)
    #print (len(x))
    if len(x)>1: 
        print ('Country code for this location is ' + x + '.')
    else: 
        print ('Country code cannot be found.')
    x = ('')
    #Resets the country code so that it can be reupdated when you enter a new location. 


#Tested the code on Paris (printed out FR), [Seattle, WA] (printed out US), China (printed out CN), Brazil (printed out BR), London (printed out GB), Cape Town (printed out ZA), 
#Atlantic Ocean (printed out 'cannot be found'), and the Black Sea (printed out 'cannot be found')